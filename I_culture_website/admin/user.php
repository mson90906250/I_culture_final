	...
	if(isset($_GET['add'])){ 
		...
		if($_POST){
			...
			if(!empty($errors)){
				...
				$db->query("INSERT INTO admin(username,email,password,type) VALUES ('$name','$email','$hashed_password','$permission')");
		...
		<form action="user.php?add=1" method="post">
			...
			<div class="form-group col-md-6">
				...
				<select name="permission" id="permission" class="form-control">
					...
					<option value="m" <?= (($permission == "m")?' selected':'') ?>>Modifier</option>
					<option value="s" <?= (($permission == "s")?' selected':'') ?>>Shop Owner</option>
				</select>
			</div>
...
<?php }else{
		$usersQuery = $db->query("SELECT * FROM admin ORDER BY username");

 ?>

<h2 class="text-center">Users</h2>
<a href="user.php?add=1" class="btn btn-success pull-right" id="add_product_btn">Add A New User</a>
<hr>
			...
			<tr>
				<td>
					<?php if($user['id'] != $user_data['id']): ?>
						<a href="user.php?delete=<?= $user['id'] ?>" class="btn btn-xs btn-default" onclick="return confirm('Are you sure that you want to delete this user?');">
